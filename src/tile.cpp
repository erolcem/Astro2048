...
void Tile::draw(sf::RenderWindow &window, int x, int y) {
  ...
  sf::RectangleShape tileDrawn(
      sf::Vector2f(TILESIZE - 2 * PADDING, TILESIZE - 2 * PADDING));
  ...
  textValue.setPosition(x * TILESIZE + PADDING + 1 * TILESIZE / 3,
                        y * TILESIZE + PADDING + TILESIZE / 3);
  ...
  if (colourMap.count(value) > 0 && !isEmpty && value < 128) {

    window.draw(textValue);

  } else if (colourMap.count(value) >= 0 && !isEmpty && value >= 128) {
    textValue.setPosition(x * TILESIZE + PADDING + 1 * TILESIZE / 5,
                          y * TILESIZE + PADDING + TILESIZE / 3);
    window.draw(textValue);
    textValue.setPosition(x * TILESIZE + PADDING + 1 * TILESIZE / 3,
                          y * TILESIZE + PADDING + TILESIZE / 3);
